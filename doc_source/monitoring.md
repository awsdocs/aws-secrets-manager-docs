...
All Secrets Manager actions are logged by CloudTrail and are documented in the [AWS Secrets Manager API Reference](http://docs.aws.amazon.com/secretsmanager/latest/apireference/)\. For example, calls to the `CreateSecret`, `GetSecretValue` and `RotateSecret` sections generate entries in the CloudTrail log files\. 
